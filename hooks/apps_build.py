...
class AppsBuildHook(BuildHookInterface):
    ...
    - Copy package.json, package-lock.json, cli.js, and index.js for Node.js dependencies
    ...
    def finalize(
        ...
        node_files = ["package.json", "package-lock.json", "cli.js", "index.js", "index.d.ts", "config.d.ts"]
        for file_name in node_files:
            file_path = Path(file_name)
            if file_path.exists():
                self.app.display_info(f"Copying {file_path} to {build_dir}")
                shutil.copy(file_path, build_dir)
            else:
                self.app.display_info(f"{file_path} does not exist, skipping copy")
